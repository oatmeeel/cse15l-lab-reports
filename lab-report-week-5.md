...
lists every file that ends in .txt (like the previous example but with many many more files) in ./technical and prints them in a new file 'findresults.txt' in the current directory.
...
example command:
```
find ./technical -type f > findresults.txt
```

output:

lists everything with the type f (for file) in ./technical and prints them in a new file 'findresults.txt' in the current director), or replaces the current file named 'findresults.txt' with the contents of the new file. similar to 'find ./technical -name *.txt > findresults.txt' except it is not limited to .txt files.
...
example command:
```
find ./technical -type f name bio* > findresults.txt
```
...
example command:
```
find ./technical -name chapter-1.txt -exec cat {} \;
```

example command:
```
find ./technical -type d -exec ls {} \;
```
